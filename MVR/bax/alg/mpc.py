...
class MPC(BatchAlgorithm):
    ...
    def reset_CEM(self, shift_actions=[]):
        self.mean = np.concatenate([self.mean[self.params.actions_per_plan:],
                                    np.zeros((self.params.actions_per_plan, self.params.action_dim))])#use results from previous iteration and extend horizon with new elements by adding zeroes
        self.var = np.ones_like(self.mean) * ((self.params.action_upper_bound - self.params.action_lower_bound) /
    ...
    def shift_samples(self, all_returns, all_states, all_actions, all_rewards):
        ...
        short_shifted_actions = all_actions[keep_indices, self.params.actions_per_plan:, :]#using previous planning for the old steps
        ...
        self.shifted_actions = np.concatenate([short_shifted_actions, new_actions], axis=1)#add randomly sampled actions for the new steps
        self.current_t_plan = 0
