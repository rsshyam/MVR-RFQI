...
class RMPC(BatchAlgorithm):
    """
    An algorithm for robust model-predictive control. Here, the queries are concatenated states
    and actions and the output of the query is the perturbed uncertain next state.  We need the reward
    ...
    def set_params(self, params):
        ...
        self.params.name = getattr(params, "name", "RMPC")
        ...
        self.params.para_t=getattr(params,"para_t",10)
        self.params.Delta=getattr(params,"Delta",0.01)
        ...
        self.params.traj_noise=getattr(params,'traj_noise', None)
    ...
    def initialize(self, samples_to_pass=[]):
        ...
        super().initialize()#initialize exe_path from algo.py

        # set up initial CEM distribution
        self.mean = np.zeros((self.params.planning_horizon, self.params.action_dim))#defined above in set_params
        self.var = np.ones_like(self.mean) * ((self.params.action_upper_bound - self.params.action_lower_bound) /
                                              self.params.initial_variance_divisor) ** 2#defined above in set_params
        ...
        self.traj_samples = iCEM_generate_samples(initial_nsamps,
                                                  ...
                                                  self.params.action_upper_bound)#util/control_util
        # self.traj_samples = list(self.traj_samples)
        # self.traj_samples += samples_to_pass
        # this one is for prin
        #print(self.traj_samples)
        self.traj_samples=np.repeat(self.traj_samples,self.params.para_t,axis=0)#possible error
        #self.traj_noise=np.random.normal(0,1,size=(self.params.planning_horizon,self.params.para_t,self.params.obs_dim))
        #self.traj_noise=self.traj_noise[t:t+self.params.planning_horizon,:,:]
        #print(self.traj_noise.shape)
        ...
        if self.params.start_obs is not None:
            ...
            self.current_obs = self.params.start_obs#defined in execute_mpc
        else:
            logging.debug('Sampling start obs from env')
            self.current_obs = self.params.env.reset()
        self.iter_num = 0
        self.samples_done = False
        self.planned_states = [self.current_obs]#from execute_mpc/given
        self.planned_actions = []
        self.planned_rewards = []
        self.saved_states = []
        self.saved_actions = []
    ...
    def get_next_x_batch(self):
        ...
        """
        if len(self.exe_path.x) > 0:#after first run batch is added to path, so non-zero
            self.process_prev_output()#only if exe_path is non-empty
        ...
        if self.samples_done and self.iter_num + 1 == self.params.num_iters:
            ...
            self.resample_iCEM()
        #initially samples_done is false, so directly return
        return self.get_sample_x_batch()

    def get_sample_x_batch(self):
        actions = self.traj_samples[:, self.current_t_plan, :]#from icem_generate
        if self.current_t_plan == 0:#initally 0
            obs = np.tile(self.current_obs, (actions.shape[0], 1))
        else:
            obs = self.traj_states[-1]#updated in prev_output
        queries = np.concatenate([obs, actions], axis=1)
        if self.params.project_to_domain:#always set to false(i guess)
            raise NotImplementedError()
            queries = project_to_domain(queries, self.params.domain)
        batch = list(queries)
        return batch

    def resample_iCEM(self):#save all actions/rewards in current iteration and sample/reinitialize for next iteration
        self.iter_num += 1#initially zero from initialize()
        
        #worstcase_traj/rewards
        returns=compute_return(np.array(self.traj_rewards).T, self.params.discount_factor)#transpose from horizon*traj to traj*horizon and compute reward
        #print(returns)
        returns=returns.reshape((-1,self.params.para_t))#reshape returns to an extra dimension such that the same action sequences' returns can be compared
        #print(returns)
        old_nsamps=returns.shape[0]#number of action sequences
        rbt_ind=np.argmin(returns,axis=1)#worst case return indices for each action sequence
        returns=returns[np.arange(old_nsamps),rbt_ind]
        #print(returns)
        #print(self.traj_rewards,"rewards")
        self.traj_rewards=np.array(self.traj_rewards).T.reshape((-1,self.params.para_t,self.params.planning_horizon))
        #print(self.traj_rewards,"rewards")
        self.traj_rewards=self.traj_rewards[np.arange(old_nsamps),rbt_ind]
        #self.traj_rewards=np.mean(self.traj_rewards,axis=1)
        #print(self.traj_rewards,"rewards")
        self.traj_rewards=self.traj_rewards.squeeze().T
        #print(self.traj_rewards,"rewards")
        #print(self.traj_samples,"samples")
        self.traj_samples=self.traj_samples.reshape((-1,self.params.para_t,self.params.planning_horizon,self.params.action_dim))
        #print(self.traj_samples,"samples")
        self.traj_samples=self.traj_samples[np.arange(old_nsamps),rbt_ind]
        #self.traj_samples=np.mean(self.traj_samples,axis=1)
        #print(self.traj_samples,"output")
        #print(self.traj_samples.shape,"output")
        #print(self.traj_samples,"samples")
        #print(self.traj_states,"states")
        self.traj_states=np.array(self.traj_states).transpose((1, 0, 2)).reshape((-1,self.params.para_t,self.params.planning_horizon,self.params.obs_dim))
        #print(self.traj_states,"states")
        self.traj_states=self.traj_states[np.arange(old_nsamps),rbt_ind]
        #self.traj_states=np.mean(self.traj_states,axis=1)
        #print(self.traj_states,"states")
        self.traj_states=self.traj_states.transpose((1,0,2))
        #print(self.traj_states,"states")
        ...
        if len(self.saved_rewards) > 0:#initially 0
            ...
            all_actions = self.traj_samples#from icem_generate in initialize
        ...
        self.traj_samples = samples#updated after every iteration
        # self.traj_samples = list(samples)
        #print(self.traj_samples,"expect")
        #print(self.traj_samples.shape,"expect")
        #repeat samples
        self.traj_samples=np.repeat(samples,self.params.para_t,axis=0)#possible error
        
        self.traj_states = []
        self.traj_rewards = []
        self.samples_done = False

    def process_prev_output(self):#calculates rewards for action samples and new states
        n_samps = len(self.traj_samples)#computed in intitialize() from icem_generate
        #print(n_samps)
        new_x = np.array(self.exe_path.x[-n_samps:])
        new_y = np.array(self.exe_path.y[-n_samps:])
        if self.current_t_plan == 0:#initially 0 from initialize()
            obs = np.tile(self.current_obs, (n_samps, 1))
        else:
            obs = self.traj_states[-1]
        delta = new_y if self.params.reward_function else new_y[:, 1:]
        #print(delta,"before noise")
        #print(np.reshape(np.repeat(self.params.traj_noise[self.current_t+self.current_t_plan].T,int(n_samps/self.params.para_t),axis=0),(-1,self.params.obs_dim)),"noise")
        delta = delta#+np.reshape(np.repeat(self.params.traj_noise[self.eval_t+self.current_t_plan].T,int(n_samps/self.params.para_t),axis=0),(-1,self.params.obs_dim))*self.params.Delta
        #print(delta,"after noise")
        new_obs = self.update_fn(obs, delta)#delta and y are change in state, hence add delta with old state
        new_obs= new_obs+np.reshape(np.repeat(self.params.traj_noise[self.eval_t+self.current_t_plan].T,int(n_samps/self.params.para_t),axis=0),(-1,self.params.obs_dim))*self.params.Delta
        self.traj_states.append(new_obs)#initialized as None in initialize()
        if self.params.reward_function:
            ...
            rewards = new_y[:, 0]
        self.traj_rewards.append(rewards)#initialized in initialize()
        self.current_t_plan += 1#initialized as 0 in initialize()
    ...
    def save_planned_actions(self):
        ...
        # and adjust the time and current state according to actions_per_plan which decides how many actions we decide in every plan--4
    ...
    def reset_CEM(self, shift_actions=[]):
        self.mean = np.concatenate([self.mean[self.params.actions_per_plan:],
                                    np.zeros((self.params.actions_per_plan, self.params.action_dim))])#why not current_t?
        self.var = np.ones_like(self.mean) * ((self.params.action_upper_bound - self.params.action_lower_bound) /
        ...
        self.traj_samples = np.concatenate([self.traj_samples, shift_actions], axis=0)#why concatenate with shift actions?
        
        
        #probably repeat here
        self.traj_samples=np.repeat(self.traj_samples,self.params.para_t,axis=0)#possible error
    ...
    def shift_samples(self, all_returns, all_states, all_actions, all_rewards):
        ...
        short_shifted_actions = all_actions[keep_indices, self.params.actions_per_plan:, :]#why not current_t?
        new_actions = np.array([[self.params.env.action_space.sample() for _ in range(self.params.actions_per_plan)] for i in range(n_keep)])#length of shifted actions is less so add by sampling
    ...
    def execute_mpc_mult_act(self, obs, f, samples_to_pass=[], return_samps=False, actions_per_plan=1):
        """Run MPC on a state, returns the optimal action."""
        old_horizon = self.params.env_horizon
        old_start_obs = self.params.start_obs
        old_app = self.params.actions_per_plan
        self.params.actions_per_plan = actions_per_plan
        self.params.env_horizon = 1###should be changed?
        self.params.start_obs = obs
        if len(self.exe_path.x) > 0:
            self.old_exe_paths.append(self.exe_path)
        self.initialize(samples_to_pass=samples_to_pass)
        ...
        exe_path, output = self.run_algorithm_on_f(f)
        self.is_test = False
        actions = output[1][0:actions_per_plan]
        self.eval_t=self.eval_t+1
        #print(self.eval_t)
        self.params.env_horizon = old_horizon
        self.params.start_obs = old_start_obs
        self.params.actions_per_plan = old_app
