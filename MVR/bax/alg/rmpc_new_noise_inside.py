...
class RMPC(BatchAlgorithm):
    ...
    def process_prev_output(self):#calculates rewards for action samples and new states
        ...
        #print(np.reshape(np.repeat(self.traj_noise[self.current_t_plan].T,int(n_samps/self.params.para_t),axis=0),(-1,self.params.obs_dim)),"noise")
        delta = delta+np.reshape(np.repeat(self.traj_noise[self.current_t_plan].T,int(n_samps/self.params.para_t),axis=0),(-1,self.params.obs_dim))*self.params.Delta
