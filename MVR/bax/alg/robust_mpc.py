...
class RMPC(BatchAlgorithm):
    ...
    def initialize(self, samples_to_pass=[]):
        ...
        # this one is for CEM
        self.traj_samples=self.traj_samples.repeat(samples,self.params.para_t,axis=0)#possible error
    ...
    def resample_iCEM(self):#save all actions/rewards in current iteration and sample/reinitialize for next iteration
        ...
        self.traj_rewards=np.array(self.traj_rewards).T.reshape((-1,self.params.para_t,self.params.horizon))
        self.traj_rewards=self.traj_rewards[np.arange(old_nsamps),rbt_ind]
        self.traj_rewards=self.traj_rewards.squeeze().T
        self.traj_samples=self.traj_samples.reshape((-1,self.params.para_t,self.params.horizon,self.params.act_dim))
        self.traj_samples=self.traj_samples[np.arange(old_nsamps),rbt_ind]
        self.traj_states=self.traj_states.transpose((1, 0, 2)).reshape((-1,self.params.para_t,self.params.horizon,self.params.obs_dim))
    ...
    def reset_CEM(self, shift_actions=[]):
        ...
        self.traj_samples=self.traj_samples.repeat(self.traj_samples,self.params.para_t,axis=0)#possible error
