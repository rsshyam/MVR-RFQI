...
class LavaPathEnv(gym.Env):
    ...
    def __init__(self, f_dynamics=None):
    ...
    def step(self, action):
        # Action is [F_x, F_y]
        if self.f_dynamics:
            state=self.construct_obs()
            norm_state=self.normalize_obs(state)
            #action to be normalized
            #print(state,norm_state)
            #print(norm_state)
            norm_delta_s = self.f_dynamics(np.reshape(np.append(norm_state,action),(1,-1)))
            #print(norm_delta_s)
            ...
            # print(unnorm_next_state)
            unnorm_next_state=np.squeeze(unnorm_next_state)
            new_x=unnorm_next_state[0:2]
            new_x_dot=unnorm_next_state[2:4]
            #x, x_dot, unnorm_theta, theta_dot = np.squeeze(unnorm_next_state)
            #theta = angle_normalize(unnorm_theta)
            # self.state = (x,x_dot,theta,theta_dot)

            #truemodel
            tru_a = action / self.mass

            tru_new_x_dot = tru_a * self.dt + self.x_dot
            tru_new_x = 0.5 * tru_a * self.dt**2 + self.x_dot * self.dt + self.x
            
            if(tru_new_x[0] < self.playable_area.low[0]):
                tru_new_x[0] = self.playable_area.low[0]
                tru_new_x_dot[0] = 0
            # If we're east of the playable area
            elif(tru_new_x[0] > self.playable_area.high[0]):
                tru_new_x[0] = self.playable_area.high[0]
                tru_new_x_dot[0] = 0

            # If we're south of the playable area
            if(tru_new_x[1] < self.playable_area.low[1]):
                tru_new_x[1] = self.playable_area.low[1]
                tru_new_x_dot[1] = 0
            # If we're right of the playable area
            elif(tru_new_x[1] > self.playable_area.high[1]):
                tru_new_x[1] = self.playable_area.high[1]
                tru_new_x_dot[1] = 0

            # Lava wall bound checking
            if(self.enable_lava_walls):
                tru_new_x, tru_new_x_dot = self.check_lava_wall_collision(tru_new_x, tru_new_x_dot)

            tru_state=np.concatenate([tru_new_x, tru_new_x_dot])
        ...
        if self.f_dynamics:
            info={((tru_state - unnorm_next_state)**2).mean(axis=0)}
        else:
            info = {}
