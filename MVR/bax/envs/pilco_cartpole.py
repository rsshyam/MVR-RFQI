...
class CartPoleSwingUpEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if self.f_dynamics:
            ...
            #5. always normalize new angles
            #6. wrap doesnt change the state, just unnormalizes action sends to this step, gets unnormalized state and normalizes it
            #if type(action)=='list':
            ...
            #print(action)
            action=action/self.force_mag ##makes it between -1 and 1#normalize as f_dynamics requires normalized state-action pair
            #print(action)
    ...
    #to rewrite both these function as self.observation_space is not defined
