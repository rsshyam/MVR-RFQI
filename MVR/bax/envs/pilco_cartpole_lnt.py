...
import numpy as np
from pilco_cartpole.py import CartPoleSwingUpEnv
...
class CartPoleSwingUpEnvlnt(CartPoleSwingUpEnv):
    metadata = {
    ...
    def step(self, action):
        ...
        if self.f_dynamics:
            self.state = self.f_dynamics(np.concatenate((state, action), axis=1))
        else:
            s = math.sin(theta)
            c = math.cos(theta)
