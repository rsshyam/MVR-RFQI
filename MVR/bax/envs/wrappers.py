...
class NormalizedEnv(Env):
    ...
    def step(self, action):
        ...
        if 'delta_obs' in info:
            ...
            norm_delta_obs = unnorm_delta_obs / self.unnorm_obs_space_size * 2#s between -1 and 1, hence change in s is between -2 and 2
    ...
    def unnormalize_action(self, action):
        #write separate case scalar action
        if action.ndim == 1:
...
def make_update_obs_fn(env, teleport=False):
    ...
    def update_obs_fn(x, y):
        start_obs = x[..., :obs_dim]#last -- columns of all rows
        delta_obs = y[..., -obs_dim:]#first -- columns of all rows
        output = start_obs + delta_obs#y models change in state and not state--hence the addition
        #np.clip(output,env.observation_space.low,env.observation_space.high,out=output)
        if not teleport:
            return output
