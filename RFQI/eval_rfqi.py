...
def eval_rfqi(state_dim, action_dim, min_action, max_action, hard, paths,
    ...
    wandb.init(
        ...
        project=f"robust rl {perturbed_env}_2",name=f'{args.comment}_rfqi{args.rho}_eps{args.data_eps}_{args.nsamples}',
    ...
    if args.env == 'Pendulum-v1':
        ...
        if init_angle_mag != 0.2:
            save_path = save_path+f'_init_angle_mag_{init_angle_mag}'
        ...
        for p in ps_g:
            ...
            for _ in range(eval_episodes):
                ...
                episode_reward = 0.0
                i=0
                while not done:#runs only for 200 bcos of timelimit wrapper in gym/registration
...
if __name__ == "__main__":
    ...
    if args.d4rl == 'False' and args.mixed == 'False':
        if args.dtalrnt == 'True':
            load_path = f'RFQI_{args.env}_rho{args.rho}_dataeps{args.data_eps}_datapol{args.gendata_pol}_nsamples_{args.nsamples}_{args.comment}'
            save_path = f'RFQI_{perturbed_env}_rho{args.rho}_dataeps{args.data_eps}_datapol{args.gendata_pol}_nsamples_{args.nsamples}_{args.comment}'
        else:
