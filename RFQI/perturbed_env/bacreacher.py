...
class BACReacherEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    def __init__(self, tight=False, f_dynamics=None):
        ...
        mujoco_env.MujocoEnv.__init__(self, "%s/assets/bacreacher.xml" % dir_path, 2)
    ...
    def reset(self, seed=None, obs=None):
        if seed is not None:
            old_obs=super().reset(seed=seed)
        else:
            old_obs = super().reset()
        if obs is None:
            if self.f_dynamics:
                self.f_dynamics_state=old_obs
            return old_obs
        ...
        if self.f_dynamics:
            self.f_dynamics_state=check_obs
